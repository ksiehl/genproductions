...
cat<<-EOF
	...
	unset CXX CC FC
	# Run
	iscmsconnect=1 bash -x cpodd_gridpack_generation.sh "${card_name}" "${card_dir}" "${workqueue}" CODEGEN "${scram_arch}" "${cmssw_version}"
	exitcode=\$?
...
card_name="aTGCstudy_cpodd_ww_wvmass600to800" #$1
#card_dir="/local-scratch/ksiehl/Cards/$card_name" #$2
card_dir="cards/production/pre2017/13TeV/WVTolnujj_cpodd/$card_name" #$2
#card_dir="card_single_odd_coupling_noaddproc/$card_name"
#card_dir="card_single_odd_coupling_addproc/$card_name"
#card_dir="card_multi_odd_coupling_noaddproc/$card_name"
workqueue="condor"
...
iscmsconnect=1 bash -x cpodd_gridpack_generation.sh ${card_name} ${card_dir} ${workqueue} INTEGRATE ${scram_arch} ${cmssw_version}
...
#iscmsconnect=1 bash -xe cpodd_gridpack_generation.sh ${card_name} ${card_dir} ${workqueue} MADSPIN ${scram_arch} ${cmssw_version}
